...
public class FunBean implements InitializingBean, DisposableBean {
    ...
    public void afterPropertiesSet() {
        logger.info("<<< (2) calling 'afterPropertiesSet' >>>");
    }

    @Override
    public void destroy() {
        logger.info("<<< (5) calling 'destroy' >>>");
    }

    @PostConstruct
    private void postConstruct() {
        logger.info("<<< (1) calling 'postConstruct' >>>");
    }

    @PreDestroy
    private void preDestroy() {
        logger.info("<<< (4) calling 'preDestroy' >>>");
    }

    public void initializationMethod() {
        logger.info("<<< (3) calling 'initializationMethod' >>>");
    }

    public void destroyMethod() {
        logger.info("<<< (6) calling 'destroyMethod' >>>");
    }
}
